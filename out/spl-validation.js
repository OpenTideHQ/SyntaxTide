 ...
 */
function validateCommandParametersNew(commandName, commandLine, context, commandStartPos, logger) {
    const diagnostics = [];
    if (logger) {
    ...
    if (!cmd || !cmd.parameters || cmd.parameters.length === 0) {
        if (logger)
            logger(`  No parameters defined, exiting`);
    ...
    if (commandLineStart >= 0) {
        ...
        if (logger)
            logger(`  Using direct indexOf, baseOffset: ${baseOffset}`);
    }
    else {
        ...
        if (cmdPos >= 0) {
            ...
            if (pipePos >= 0) {
                ...
                if (logger)
                    logger(`  Using pipe calculation, baseOffset: ${baseOffset}`);
            }
            else {
                baseOffset = cmdPos;
                if (logger)
                    logger(`  Using cmdPos, baseOffset: ${baseOffset}`);
            }
        }
    }
...
function validateSPLLine(line, lineNumber, documentUri, charOffset = 0, availableVariables, logger) {
    const diagnostics = [];
