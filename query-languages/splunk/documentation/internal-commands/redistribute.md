...
The `redistribute` command requires a [distributed search](https://docs.splunk.com/Splexicon:Distributedsearch) environment where indexers have been configured to operate as intermediate reducers.

You can use the `redistribute` command only once in a search.

## Syntax

redistribute [num\_of\_reducers=<int>] [<by-clause>]
...
Default: The default value for `num_of_reducers` is controlled by three settings in the `limits.conf` file:`maxReducersPerPhase`, `winningRate`, and `reducers`. If these settings are not changed, by default the Splunk software sets `num_of_reducers` to 50 percent of your indexer pool, with a maximum of 4 indexers. See [Usage](/splunk-enterprise/search/spl-search-reference/10.0/internal-commands/redistribute#f7614217_e135_420b_92c4_52e167c92fee__Usage) for more information.

by-clause

Syntax: BY <field-list>

Description: The name of one or more fields to group by. You cannot use a wildcard character to specify multiple fields with similar names. You must specify each field separately. See [Using the by-clause](/splunk-enterprise/search/spl-search-reference/10.0/internal-commands/redistribute#id_0988423c_ae57_4cbf_81bc_8a2a136e9309__redistribute) for more information.
...
For information about managing parallel reduce processing at the indexer level, including configuring indexers to operate as intermediate reducers, see [Overview of parallel reduce search processing](/en/?resourceId=Splunk_DistSearch_Parallelreduceoverview), in the *Distributed Search* manual.
...
The `redistribute` command supports only [streaming commands](https://docs.splunk.com/Splexicon:Streamingcommand) and the following nonstreaming commands:
...
The `redistribute` command is an [orchestrating command](https://docs.splunk.com/Splexicon:Orchestratingcommand), which means that it controls how a search runs. It does not focus on the events processed by the search. The `redistribute` command instructs the distributed search query planner to convert centralized streaming data into distributed streaming data by distributing it across the intermediate reducers.

For more information about command types, see [Types of commands](/en/?resourceId=Splunk_Search_Typesofcommands) in the *Search Manual*.
...
Searches that use the `redistribute` command ignore all data on the search head. If you plan to use the `redistribute` command, the best practice is to forward all search head data to the indexer layer. See [Best Practice: Forward search head data to the indexer layer](/en/?resourceId=Splunk_DistSearch_Forwardsearchheaddata) in the *Distributed Search* manual.
...
index=main | redistribute | transaction referer\_domain | search eventcount>500 | sitimechart count by referer\_domain | search referer\_domain=\*.net | timechart count by referer\_domain
