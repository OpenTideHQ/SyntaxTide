...
Creates one or more relative time fields and adds the field or fields to returned events. Each added relative time field provides a human-readable value of the difference between "now" (the start time of the search) and the [timestamp](https://docs.splunk.com/Splexicon:Timestamp) value of a corresponding field in the returned event. Human-readable values look like `5 days ago`, `1 minute ago`, `2 years ago`, and so on.
...
The `reltime` command is a distributable streaming command. See [Command types](/splunk-enterprise/search/spl-search-reference/10.0/quick-reference/command-types#a7189144_fdbc_4890_949d_4657d94c2778__Command_types).
...
... | reltime timefield="max\_time,min\_time,current\_time"
...
... | reltime timefield="max\_time,min\_time" prefix=usr\_prefix\_

## See also

[convert](/en/?resourceId=Splunk_SearchReference_Convert)
